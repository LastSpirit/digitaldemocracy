    ...
    <Box className={classes.bigCardContainer}>
      <Box className={classes.mainHeader}>
        ...
        <Box className={classes.date}>
          <Typography className={classes.text}>{'S 21.10.2021 '}</Typography>
          <Typography className={classes.text}>{'Po 21.11.2022'}</Typography>
        </Box>
      ...
      <Box className={classes.content}>
        ...
        >
          <Box sx={{ marginBottom: '10px' }}>
            <Typography className={classes.bigTitle}>
              {/* {title} */}
              Государственной Думы Федерального Собрания Российской Федерации восьмого созыва
            </Typography>
          </Box>
          <Box className={classes.cardNames}>
            ...
            <Typography sx={{ padding: 0 }} className={classes.clickableText}>
              Российская федерация Республика Северная Осетия - Алания Владикавказ
            </Typography>
          </Box>
        </Box>
        {/* <Box className={classes.imageContainer} onClick={handleNews}>
          {image.length > 1 ? (
            image
              .map((elem) => <img src={checkImg(elem)} key={elem} alt="news" className={classes.image} />)
              .splice(0, 2)
          ) : (
