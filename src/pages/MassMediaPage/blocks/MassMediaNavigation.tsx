...
const MassMediaNavigation = () => {
  const { massMediaId }: { massMediaId: string } = useParams();
  const {
    location: { pathname },
  } = useHistory();
        ...
        <div className={styles.tabList}>
          {MassMediaTabs.map(({ title, id }, index) => (
            <Link
              ...
              to={(location: any) => ({ ...location, pathname: `/mass-media/${massMediaId}/${id}` })}
            >
              {title}
            </Link>
          ))}
      ...
      <div className={styles.tabContent}>
        <Switch>
          {MassMediaTabs.map(({ id, component }) => (
            <Route key={id} path={`/mass-media/:massMediaId/${id}`} component={component} />
          ))}
          <Redirect to={`/mass-media/:massMediaId/${MassMediaTabs[0].id}`} />
        </Switch>
      </div>
    </div>
  );
