...
const fetchWidgetLink: APIRequest<WidgetLinkRequest, WidgetLinkResponse> = (args) => {
  ...
  return callAPI({ url: `news/widgetLink/${id}${(topicId && page) ? `?page=${page}&topic_id=${topicId}` : (!topicId && page) ? `?page=${args.payload.page}` : topicId && !page ? `?topic_id=${topicId}` : ''}`, config: { method: 'GET' }, ...args });
};
